...
const NavItem = ({ icon, label, active, onClick }: { icon: React.ReactNode; label: string; active: boolean; onClick: () => void }) => (
  <button onClick={onClick} className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-colors font-semibold text-[0.9rem] ${active ? "bg-[var(--color-button)] text-white shadow-sm" : "text-text-muted hover:bg-slate-800/50 hover:text-text-primary"}`}>
    {icon} {label}
  </button>
);
...
const Overview = ({ onAddNew, onEdit, onDelete, properties }: { onAddNew: () => void; onEdit: (id: string | number) => void; onDelete: (id: string | number) => void; properties: Property[] }) => {
  ...
  <div className="p-6 flex flex-col gap-6 w-full max-w-5xl mx-auto">
    <div className="flex flex-col gap-3">
      ...
      <button onClick={onAddNew} className="bg-[var(--color-button)] text-white font-bold px-5 py-2.5 rounded-xl shadow-sm hover:bg-[var(--color-button)]/90 active:scale-95 transition-all flex items-center gap-2 justify-center w-full">
            ...
            {properties.map((h) => (
              <tr key={h.id} className="border-b border-[var(--color-border)] last:border-0">
                ...
                <td className="py-4 flex items-center justify-end gap-2">
                  <button onClick={() => onEdit(h.id)} className="w-8 h-8 rounded-lg bg-[var(--color-accent-muted)]/30 text-[var(--color-accent)] flex items-center justify-center hover:bg-[var(--color-button)] hover:text-white transition-colors" title="Edit"><Edit2 size={14} /></button>
  ...
  return (
    <div className="p-6 flex flex-col gap-6 w-full mx-auto pb-20">
      ...
      <div className="flex gap-2 mb-2">
        {[1, 2, 3, 4].map((s) => (
          <div key={s} className={`h-2 flex-1 rounded-full ${s <= step ? "bg-[var(--color-button)]" : "bg-[var(--color-border)]"}`} />
        ))}
      </div>
      ...
      <form className="flex flex-col gap-5" onSubmit={(e) => { e.preventDefault(); if (step < 4) { handleNext(); } else { onSave(formData as ManagerPropertyForm); } }}>
        ...
        {step === 1 && (
          <div className="bg-[var(--color-surface)] p-6 rounded-2xl shadow-sm border-transparent border flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-2">
            ...
            {formData.nearestCampus && (
              <div className="flex items-start gap-3 bg-[var(--color-button)]/5 border border-[var(--color-accent)]/20 rounded-xl px-4 py-3 animate-in fade-in">
        ...
        <div className="flex items-center justify-between pt-4 pb-12">
          ...
          {step < 4 ? (
            <button type="button" onClick={handleNext} className="bg-[var(--color-button)] text-white font-bold px-8 py-2.5 rounded-xl shadow-sm hover:bg-[var(--color-button)]/90 active:scale-95 transition-all">Next</button>
          ) : (
            <button type="submit" className="bg-[var(--color-button)] text-white font-bold px-8 py-2.5 rounded-xl shadow-sm hover:bg-[var(--color-button-hover)] active:scale-95 transition-all flex items-center gap-2">

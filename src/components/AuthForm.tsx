    ...
    <div className="w-full flex-1 flex flex-col justify-center animate-in fade-in slide-in-from-right-4 duration-300">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
        ...
        <button
          type="submit"
          disabled={isLoading}
          className={`w-full text-white font-bold text-[0.95rem] py-4 rounded-[14px] shadow-lg transition-transform active:scale-[0.98] mt-2 flex justify-center items-center bg-[var(--color-button)] hover:bg-[var(--color-button-hover)] ${isLoading ? 'opacity-70 cursor-not-allowed' : ''}`}
        >
          {isLoading ? (
